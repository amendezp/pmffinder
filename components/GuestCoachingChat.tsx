...
export function GuestCoachingChat({ stageNumber }: Props) {
  ...
  useEffect(() => {
    ...
    setMessages(state.chats[stageNumber] ?? []);
  }, [stageNumber]);
  ...
  async function send() {
    ...
    setDraft("");
    setStreaming(true);
    const userMsg: GuestChatMessage = { role: "user", content: text };
    const next: GuestChatMessage[] = [...messages, userMsg, { role: "assistant", content: "" }];
    setMessages(next);
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        const final = [...messages, userMsg, { role: "assistant" as const, content: `[error: ${res.status}]` }];
        setMessages(final);
        setChat(stageNumber, final);
        return;
      }
      ...
      >
        <header className="border-b border-ink-700/15 px-4 py-3">
          ...
          <p className="text-xs text-ink-700/80">
            Demo mode — chat history is stored in your browser only.
          </p>
        </header>
